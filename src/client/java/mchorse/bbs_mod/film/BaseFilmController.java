...
public abstract class BaseFilmController
{
    ...
    private static void renderAxes(String bone, boolean local, TransformSpace space, Matrix4f gizmoView, StencilMap stencilMap, Form form, IEntity entity, float transition, MatrixStack stack, FormFrameCache frame)
    {
        ...
        if (matrix != null)
        {
            ...
            if (stencilMap == null)
            {
                /* Snapshot the placement only; the visual draws later in the panel's UI pass
                 * (Gizmo#renderInterface — rebuilt on manual render passes), so its translucent
                 * parts composite through the UI pipeline AND it survives a shaderpack: the pack's
                 * end-of-world composite overwrites world-phase draws that don't go through its
                 * programs, which is exactly what made the world-drawn gizmo ghost under Iris. */
    ...
    private static void renderPreviewAxes(String bone, boolean local, Form form, IEntity entity, float transition, MatrixStack stack, FormFrameCache frame)
    {
        ...
        /* TODO(1.21.11 render merge): RenderSystem.getProjectionMatrix() was removed; the ortho-vs-
         * perspective FOV derivation it fed is deferred — fall back to the configured FOV so the gizmo
         * distance scale still works. */
        float fov = BBSSettings.getFov();
        ...
        /* TODO(1.21.11 render merge): RenderSystem.enableDepthTest() removed — depth state is pipeline-encoded. */
        stack.pop();
    }
                ...
                if (actors != null)
                {
                    ...
                    if (entityId != null)
                    {
                        ...
                        if (anEntity instanceof PlayerEntity player)
                        {
                            ...
                            /* TODO(1.21.11 render): first person teleports the player from keyframes instead of
                             * walking it, so vanilla's view-bobbing amplitude is computed from a zero velocity and
                             * stays flat. On 1.21.1 this was fixed by re-deriving strideDistance from the per-tick
                             * displacement, but 1.21.11 moved that value off ClientPlayerEntity into the entity
                             * render state, so there is no field to write. Re-do it against the render-state
                             * pipeline (the rest of the walk effects — sprinting particles, grounded — still work).
                             */
    ...
    public void startRenderFrame(float transition)
    {
        for (Map.Entry<Integer, IEntity> entry : this.entities.entrySet())
        {
            ...
            ItemUsePose.Use use = ReplayItemUse.compute(replay, tick + delta, true, user);
            ItemUsePose.Use offUse = ReplayItemUse.compute(replay, tick + delta, false, user);
    ...
    protected void renderActorNameTag(WorldRenderContext context, Replay replay, IEntity entity)
    {
        ...
        stack.push();
        stack.translate(x - camera.getCameraPos().x, y - camera.getCameraPos().y, z - camera.getCameraPos().z);
