...
public class UIKeyframeDopeSheet implements IUIKeyframeGraph
{
    ...
    private void renderSheets(UIContext context, VertexConsumer builder, Matrix3x2fc matrix, Area area, int y)
    {
        for (UIKeyframeSheet sheet : this.sheets)
        {
            if (!this.isVisible(sheet))
    ...
    private void renderSheet(UIContext context, VertexConsumer builder, Matrix3x2fc matrix, Area area, UIKeyframeSheet sheet, int y)
    {
        if (!this.isVisible(sheet))
        {
            return;
     ...
     */
    private void renderSummary(UIContext context, VertexConsumer builder, Matrix3x2fc matrix, Area area, UIKeyframeSheet sheet, int y)
    {
    ...
    private void renderSummaryMarks(UIContext context, VertexConsumer builder, Matrix3x2fc matrix, Area area, UIKeyframeSheet sheet, int my, int color)
    {
        for (UIKeyframeSheet child : sheet.children)
        {
    ...
    private void renderSheetsTopmostKeyframes(UIContext context, VertexConsumer builder, Matrix3x2fc matrix, Area area, int y)
    {
        for (UIKeyframeSheet sheet : this.sheets)
        {
            if (!this.isVisible(sheet))
    ...
    {
        UISection.renderArrow(context, cx, cy, unfolded);
    }
