 ...
 *
 * <p>1.21.11: the draws go through the shared debug {@link RenderLayer}s the IK overlay
 * registers ({@link ModelIKDebug#getLinesLayer()} / {@link ModelIKDebug#getTrisLayer()} /
 * {@link ModelIKDebug#getStencilLayer()}); depth/cull/blend are encoded by their pipelines,
 * so the old RenderSystem toggles are gone.
 */
public final class ModelPhysicsDebug
{
    ...
    private static void drawChain(MatrixStack stack, IModel model, Map<String, PivotFrame> frames, ModelPhysicsCache.CompiledChain chain, String selectedRoot, ValuePhysicsDebug config)
    {
        ...
        ModelIKDebug.flush(dots, ModelIKDebug.getTrisLayer());
    }
