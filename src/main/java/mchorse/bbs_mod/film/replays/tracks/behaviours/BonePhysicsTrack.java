...
/**
 * One bone's physics scalars — the bone's "physics" property. Behaves exactly like
 * {@link PropertyTrack}: the track drives the property's runtime value, which the physics solve
 * reads each frame; off the end of the track the runtime value is dropped and the chain runs on
 * its static scalars again. The value type is the property's own ({@link PhysicsControl}), so
 * there is no mirror class and no override map between the track and the solver.
 */
public class BonePhysicsTrack implements TrackBehaviour
{
    @Override
    public IKeyframeFactory factory(TrackId track)
    {
        return KeyframeFactories.BONE_PHYSICS;
    }
    ...
    public void apply(TrackContext context, TrackId track, KeyframeChannel channel, float tick, float blend)
    {
        ...
        if (segment != null)
        {
            ...
            bone.physics.setRuntimeValue((PhysicsControl) TrackBlend.value(channel, bone.physics.get(), segment, blend));
        }
        else if (blend >= 1F)
        {
